...
Typically, an **env file** specifies the exact version of the target runtime, the versions
of any included tools, and a list of tags to be applied to the built Docker image.
...
- `<runtime-version>` indicates the version of the runtime environment to use. It is one of the following:
  - **Full Semantic Versioning:** For example, `20.19.2`. This tag points to a specific patch version of the runtime environment.
  - **Major Version:** For example, `20`. This tag points to the latest stable release within that major version.
  - **Version Alias (`lts`):** This tag, `lts`, is dynamically determined during the build process.
    It refers to the current LTS version of the runtime environment.
...
1.  **Modify the Env File:** Update the versions of the runtime environment,
    any tools used within the image, and the desired image tags in the relevant **env file**.

    **NOTE** Avoid introducing breaking changes. As a general guideline, if bumping
    a patch version of the target runtime environment, aim to bump only the patch
    versions of the utilized tools as well.
...
4.  **Merge the Pull Request:** Once your pull request is reviewed and approved,
    squash and merged it. The CI/CD pipeline will automatically detect the changes
    in the trunk. This will initiate the workflow resulting
    in the updated image being pushed to Docker Hub.
...
- **Manual update process:** Updating image versions is a manual process, which carries
  the risk of specifying non-existent versions for the runtime environment or other tools.
...
- **Single Dockerfile template:** Currently only support one `Docker.template` file per image.
  ...
  the runtime environment without potentially breaking compatibility with older versions.
